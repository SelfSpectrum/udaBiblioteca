...
app.post('/api/login', async (req, res) => {
	...
	if ((user as any[]).length > 0) {
		req.session.userData.username = username // Store user info in the session
		req.session.userData.role = user.role // Store role info in the session
